...
PYBIND11_MODULE(_prima, m) {
                      ...
                      const py::object& options_dict)
    {
      ...
      // Determine the algorithm. We use method and algorithm interchangeably.
      ...
      // Construct the objective function depending on the whether or not a constraint function was provided
      // We do this intentionally instead of going by the provided algorithm, so that the checks in prima.c
      // that make sure that, for example, nonlinear constraints are provided when COBYLA is called or
      // that nonlinear constraints are *not* provided when an algorithm other than COBYLA is called, will
      // be triggered and return the appropriate error code, which we can then process.
      ...
      if (//rc == PRIMA_PROBLEM_SOLVER_MISMATCH_NO_NONLINEAR_CONSTRAINTS ||
          rc == PRIMA_PROBLEM_SOLVER_MISMATCH_NONLINEAR_CONSTRAINTS ||
          //rc == PRIMA_PROBLEM_SOLVER_MISMATCH_NO_LINEAR_CONSTRAINTS ||
          rc == PRIMA_PROBLEM_SOLVER_MISMATCH_LINEAR_CONSTRAINTS ||
          //rc == PRIMA_PROBLEM_SOLVER_MISMATCH_NO_BOUNDS ||
          rc == PRIMA_PROBLEM_SOLVER_MISMATCH_BOUNDS) {
        throw std::invalid_argument(prima_get_rc_string(rc));
      }
