...
def calcfc_chebyquad(x):
    ...
    y[0:n, 1] = 2*x - 1
    for i in range(1, n):
        y[0:n, i+1] = 2*y[0:n, 1] * y[0:n, i] - y[0:n, i - 1]
...
def cobyla(calcfc, m, x, f, cstrv=0, constr=0, f0=None, constr0=None, nf=None, rhobeg=None,
 ...
 INFO
   ...
   !--------------------------------------------------------------------------!
    ...
    # Write the outputs.
    # TODO: Pick up from here, to finish writing the cobyla top level routine, and then we can start testing
    # We'll be able to test up until the trust region calls, until I hear back from Zaikun about how to handle them
    return x, f


if __name__ == "__main__":
    n_chebyquad = 6
    x_chebyquad = np.array([i/n_chebyquad for i in range(1, n_chebyquad+1)])
    m = 0
    f = 0
    cstrv = 0
